...
def apply_color_overlay(intensity=0.5, blue=0, green=0, red=0):  # we change the RGB colours as individual to change frame color intensity and it is default as 0 if we didn't change when we call it.
    capture = cv2.VideoCapture(0)
    while (True):
       ret, frame = capture.read()
       frame = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)
       ...
       cv2.addWeighted(overlay, intensity, frame, 1.0, 0,frame)  # last parameter is destination source name which is same as the frame we put it before
...
def apply_sepia_filter(intensity=0.6):
    ...
    while (True):
        ...
        overlay = np.full((frame_h, frame_w, 4), sepia_bgra, dtype='uint8')  # it makes full sepia color entire frame so we make it full instead of zeros which is everything identical to frame not sepia.
...
def apply_circle_focus_blur_filter(intensity=0.2):  # FIRST WE MAKE A BLURRED FRAME THEN WE MASK CENTER OF THE FRAME THEN WE BLEND IT SO THE CENTER PART IS BLURRED.
    ...
    while (True):
        ...
        y = int(frame_h / 2)
        x = int(frame_w / 2)
        mask = np.zeros((frame_h, frame_w, 4), dtype='uint8')  # we didn't put any color because we make it zeros not full.
